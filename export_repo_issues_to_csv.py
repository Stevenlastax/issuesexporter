...
ISSUES_FOR_REPO_URL = 'https://api.github.com/repos/' + REPO + '/issues'
...
def write_issues(response, csvout):
    ...
    for issue in response.json():
        ...
        csvout.writerow([issue['number'], issue['title'].encode('utf-8'), issue['body'].encode('utf-8'), label_string.encode('utf-8'), issue['created_at'], issue['updated_at'],issue['state']])
...
def main():
    ...
    with open(csvfile, 'wb') as f:
        csvout = csv.writer(f)
        csvout.writerow(('id', 'Title', 'Body', 'Labels', 'Created At', 'Updated At','State'))
        process(csvout)
